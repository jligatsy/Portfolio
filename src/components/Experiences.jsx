...
import { FaChevronDown, FaChevronUp, FaArrowUpRightFromSquare } from "react-icons/fa6";
...
const experiences = [
  {
    ...
    description: [
      ...
      "Optimized inventory tracking for 8200+ SKUs in NetSuite and Excel, achieving 93% accuracy in data processing and inventory management.",
    ...
    tags: ["Ads Optimization", "Storefront Managements", "Product Listing", "WooCommerce","Google Analytics", "Google Search Console", "Excel", "NetSuite", "SCM-CRM-ERP"]
  },
  {
    id: 2,
    ...
    employment: "Part-Time",
    website: "www.codeninjas.com/ca-irvine",
    year: "2023 - 2024",
    logo: codeNinjasLogo,
    description: [
...
function Experiences() {
  const [activeToggle, setActiveToggle] = useState("education");
  ...
  return (
    <section id="skills" className="skills-container">
      ...
      <div className="toggle-container">
        <span className={`toggle-option ${activeToggle === "education" ? "active" : ""}`} 
          onClick={() => setActiveToggle("education")}>Education</span>
        <div className="toggle-switch" onClick={() => setActiveToggle(activeToggle === "work" ? "education" : "work")}>
          <div className={`toggle-circle ${activeToggle === "education" ? "left" : "right"}`}></div>
        </div>
        <span className={`toggle-option ${activeToggle === "work" ? "active" : ""}`} 
          onClick={() => setActiveToggle("work")}>Work Experience</span>
      </div>
        ...
        <div className="work-experience-container">
          {experiences.map((exp) => (
            <div key={exp.id} className="job-item">
              <button className={`job-header ${activeJob === exp.id ? "active" : ""}`} onClick={() => toggleJob(exp.id)}>
                <span className="job-title">{exp.title}</span>
                <span className="job-year">{exp.year} {activeJob === exp.id ? <FaChevronUp /> : <FaChevronDown />}</span>
              </button>
